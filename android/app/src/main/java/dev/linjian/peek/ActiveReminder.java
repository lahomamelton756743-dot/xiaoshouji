...
public class ActiveReminder {
    ...
    public static JSONObject config(Context ctx) {
        ...
        try {
            ...
            o.put("charge_full_enabled", p.getBoolean(AppPrefs.KEY_RULE_CHARGE_FULL, true));
            o.put("charge_full_threshold", p.getInt(AppPrefs.KEY_CHARGE_FULL_THRESHOLD, 95));
    ...
    public static String pretty(Context ctx) {
        try {
            ...
            sb.append("充电完成：").append(p.getBoolean(AppPrefs.KEY_RULE_CHARGE_FULL, true) ? "≥" + p.getInt(AppPrefs.KEY_CHARGE_FULL_THRESHOLD, 95) + "%" : "关闭").append("\n");
        ...
        try {
            ...
            if (p.getBoolean(AppPrefs.KEY_RULE_BATTERY, true)) {
                ...
                if (battery >= 0 && battery <= threshold && !charging && cooldownDue(p, "battery", now, 120 * MIN, false)) {
                    notify(ctx, "小手机低电量提醒", AppPrefs.userName(ctx) + "，手机电量 " + battery + "% 了，去充一下电。", "battery");
                }
            }

            if (p.getBoolean(AppPrefs.KEY_RULE_CHARGE_FULL, true)) {
                int battery = state.optInt("battery_percent", -1);
                boolean charging = state.optBoolean("charging", false);
                int threshold = clamp(p.getInt(AppPrefs.KEY_CHARGE_FULL_THRESHOLD, 95), 80, 100);
                if (battery >= threshold && charging && cooldownDue(p, "charge_full", now, 120 * MIN, false)) {
                    notify(ctx, "小手机充电提醒", AppPrefs.userName(ctx) + "，已经充到 " + battery + "% 了，差不多可以拔线啦。", "charge_full");
                }
            }
            ...
            if (p.getBoolean(AppPrefs.KEY_RULE_SCREEN, true) && state.optBoolean("usage_permission_ready", false)) {
                ...
                if (minutes >= threshold && oncePerDayDue(p, "screen", now)) {
                    notify(ctx, "小手机屏幕时间提醒", AppPrefs.userName(ctx) + "，今天屏幕时间约 " + formatMinutes(minutes) + " 了。眼睛休息一下吧。", "screen");
                }
            }
            ...
            if (weather != null) {
                ...
                if (note.length() > 0 && oncePerDayDue(p, "weather_" + note.hashCode(), now)) {
                    if (note.contains("雨") || note.contains("雪") || note.contains("降温") || note.contains("冷") || note.contains("高温") || note.contains("热") || note.contains("大风") || note.contains("空气") || note.contains("霾")) {
                        notify(ctx, "小手机天气提醒", WeatherState.localAdvice(note), "weather");
                    }
                }
            }
            ...
            if (cycle.optBoolean("cycle_enabled", false) && cycle.optBoolean("configured", false) && cycle.optString("date_error", "").length() == 0) {
                ...
                if (cycle.optBoolean("is_period_now", false) && oncePerDayDue(p, "cycle_period", now)) {
                    notify(ctx, "小手机生理期提醒", AppPrefs.userName(ctx) + "，现在是" + cycle.optString("current_phase", "生理期") + "，今天慢一点，注意热敷和休息。", "cycle");
                } else if (days >= 0 && days <= remindBefore && oncePerDayDue(p, "cycle_before", now)) {
                    notify(ctx, "小手机生理期提醒", AppPrefs.userName(ctx) + "，预计还有 " + days + " 天来，少吃冰的，提前准备姨妈巾和热水袋。", "cycle");
                }
            }
            ...
            for (int i = 0; i < calendarDue.length(); i++) {
                ...
                if (oncePerDayDue(p, key, now)) {
                    notify(ctx, "小手机守护日历", item.optString("banner_text", AppPrefs.userName(ctx) + "，最近有重要日子，记得回来看看。"), "calendar");
                }
            }
        } catch (Exception e) {
