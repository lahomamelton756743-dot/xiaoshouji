...
public class CompanionService extends Service {
    ...
    public static void handleCommandBody(Context ctx, String body, String serverUrl, String token) {
        try {
            ...
            if (raw instanceof String) {
                String s = (String) raw;
                if ("peek".equals(s)) { DebugState.append(ctx, "忽略旧截图命令：小手机不提供截图能力"); }
                return;
            }
    ...
    private void createNotificationChannel() { if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) { NotificationManager nm = getSystemService(NotificationManager.class); NotificationChannel channel = new NotificationChannel(CHANNEL_ID, "小手机", NotificationManager.IMPORTANCE_LOW); channel.setDescription("小手机正在等待设备来访、提醒与门禁指令"); nm.createNotificationChannel(channel); NotificationChannel reminder = new NotificationChannel(REMINDER_CHANNEL_ID, "小手机悬浮横幅提醒", NotificationManager.IMPORTANCE_HIGH); reminder.setDescription("来自小手机的悬浮横幅、生活提醒与回家模式"); reminder.setLockscreenVisibility(Notification.VISIBILITY_PUBLIC); reminder.enableVibration(true); nm.createNotificationChannel(reminder); } }
