...
public class HomeMode {
    ...
    public static void evaluate(Context ctx, JSONObject state) {
        try {
            ...
            boolean popup = CompanionService.showHomeModeNotification(ctx, "小手机回家模式", AppPrefs.userName(ctx) + "，你在 " + app + " 停了 " + ((now - start) / MIN) + " 分钟。休息一下，回到" + AppPrefs.companionName(ctx) + "这里吧。");
