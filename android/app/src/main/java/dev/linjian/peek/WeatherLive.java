...
public class WeatherLive {
    ...
    public static void refreshCoordinatesAsync(Context ctx, double lat, double lon, String city, Callback callback) {
        final Context app = ctx.getApplicationContext();
        final String label = clean(city).isEmpty() ? "当前位置" : clean(city);
        if (isFresh(app, label, DEFAULT_FRESH_MS)) {
            JSONObject cached = cached(app, label);
            if (callback != null && cached != null) callback.onResult(cached);
            return;
        }
        new Thread(() -> {
            JSONObject result;
            try {
                result = fetchCoordinates(lat, lon, label);
                AppPrefs.get(app).edit()
                        .putString(KEY_CITY, label)
                        .putString(KEY_JSON, result.toString())
                        .putLong(KEY_AT, System.currentTimeMillis())
                        .apply();
                DebugState.append(app, "天气已按定位刷新：" + label + " · " + result.optString("condition", ""));
            } catch (Exception e) {
                result = error(ScreenshotService.shortMsg(e), label);
                DebugState.append(app, "定位天气刷新失败：" + label + " · " + ScreenshotService.shortMsg(e));
            }
            if (callback != null) callback.onResult(result);
        }, "peek-weather-coordinates").start();
    }

    private static JSONObject fetchCoordinates(double lat, double lon, String label) throws Exception {
        ...
        out.put("ok", true);
        out.put("source", "open-meteo-coordinate");
        out.put("queried_city", label);
        out.put("resolved_city", label);
        ...
        out.put("summary", cardText(out, label));
        out.put("advice", advice(out, label));
        return out;
    }
    ...
    private static String get(String u) throws Exception {
        ...
        conn.setRequestProperty("User-Agent", "LittlePhone/0.6.2");
        try {
